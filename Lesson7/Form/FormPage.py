...
class FormPage:
    ...
    def fill_form(self, data):
        self.driver.find_element(*self.first_name).send_keys(
            data['first_name'])
        self.driver.find_element(*self.last_name).send_keys(
            data['last_name'])
        self.driver.find_element(*self.address).send_keys(
            data['address'])
        self.driver.find_element(*self.email).send_keys(
            data['email'])
        self.driver.find_element(*self.phone_number).send_keys(
            data['phone_number'])
        self.driver.find_element(*self.city).send_keys(data['city'])
        self.driver.find_element(*self.country).send_keys(
            data['country'])
        self.driver.find_element(*self.job_position).send_keys(
            data['job_position'])
        self.driver.find_element(*self.company).send_keys(
            data['company'])
